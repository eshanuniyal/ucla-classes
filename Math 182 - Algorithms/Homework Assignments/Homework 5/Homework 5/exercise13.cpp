...
#include <queue>
#include <string>
#include "exercise13.h"	// contains definitions for Node, Position
using namespace std;

struct Node; struct Position; struct NodeComparator;

vector<vector<int>> loadMatrix(string fileName, int dim);
// defined in auxiliary functions; loads matrix
...
int minPathSumFourWays(string fileName, int dim) {
	...
	// loading matrix
	vector<vector<int>> numMatrix = loadMatrix(fileName, dim);
	// matrix of nodes
