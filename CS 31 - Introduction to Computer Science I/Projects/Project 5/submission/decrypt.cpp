...
const char LETTERS_ARR[N_ALPHABET + 1] = "abcdefghijklmnopqrstuvwxyz"; // array of letters to compare with
...
bool decrypt(const char ciphertext[], const char crib[]) {
	// ------------------------------------------------------
	// testing for presence of words in crib
	bool words_found = false; // boolean to indicate whether there are any words in crib

	// running through crib
	for (int i = 0; i != strlen(crib); i++)
		if (isalpha(crib[i])) {
			words_found = true; // updating words_present
			break;
		}
	...
	if (static_cast<int>(strlen(simple_crib)) > MAX_MESSAGE_LENGTH)
		return false;
	...
	// match was found
		...
		for (int i = 0; i < cipher_length; i++) {
			...
			// m_char is a letter
				...
				// integer stores -1 if there's no conversion, and the index of the conversion in LETTERS_ARR if there is one
				...
				// conversion exists
					...
					cout << static_cast<char>(toupper(LETTERS_ARR[sub_exists]));
			}
		}
		// returning true
		return true;
...
bool crib_search(const char crib[], const char message[], char substitutions[]) {
	...
	// creating array parallel to LETTERS_ARR to store substitutions
	char subs[N_ALPHABET  + 1];
	subs[N_ALPHABET] = '\0';
	...
	while (true) {
		...
		while (true) {
			...
			if (isalpha(crib_char) && isalpha(m_char)) {
				...
				int char_index = detect_match(LETTERS_ARR, N_ALPHABET, crib_char); // index of crib_char in LETTERS_ARR
