...
int main()
{
    ...
    for (int i = 0; i != DEFAULT_MAX_ITEMS; i++)
        assert(insert_test.insert(string(1, char(i)), i)); // creates a map of first 240 ASCII characters and their ASCII values
    assert(insert_test.size() == 240 && !insert_test.insert("abc", 1920)); 
    ...
    assert(insert_test.size() == 239 && !insert_test.insert("1", int('1')));
        // map is no longer fully populated, but can't insert because "1" is already in map
