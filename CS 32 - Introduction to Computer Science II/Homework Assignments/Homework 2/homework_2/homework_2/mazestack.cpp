#include <stack> // defines stack object
...
bool pathExists(string maze[], int nRows, int nCols, int sr, int sc, int er, int ec) {
    ...
    coordStack.push(Coord(sr, sc)); // push starting coordinate onto coordStack
    maze[sr][sc] = 'P'; // updating current location to 'P' for path

    // iterating while stack is not empty
    while (coordStack.size() != 0) {
        Coord c_loc = coordStack.top(); // (r, c) location we're currently examining
        coordStack.pop(); // remove current coordinate from stack
