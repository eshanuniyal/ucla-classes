...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    {
        // At this point, s[0..unsorted-1] is sorted.
        // Find the right position (loc) in s[0..unsorted] for s[unsorted], which is the first entry in the unsorted region; 
        // shift, if necessary, to make room
        Student nextStudent = s[unsorted];
        int loc = unsorted;
        while ((loc > 0) && !comp(s[loc - 1], nextStudent))
        {
            // Shift s[loc - 1] to the right
            s[loc] = s[loc - 1];
            loc--;
        } // end while
        // At this point, s[loc] is where nextStudent belongs
        s[loc] = nextStudent; // Insert nextStudent into sorted region
    } // end for


    // Just to show you how to use the second parameter, we'll write code that sorts only a vector of 2 elements.
    // (This is *not* the insertion sort algorithm.)

    // Note that if comp(x,y) is true, it means x must end up before y in the final ordering.
