...
class ExpandableHashMap {
	...
	private:
		...
		std::vector<std::list<Node>> m_table;		// a vector of linked lists containing Node objects
...
void ExpandableHashMap<KeyType, ValueType>::reset() {
	...
	std::vector<std::list<Node>> m_newTable(DEFAULT_BUCKETS);
	m_table.swap(m_newTable);
...
void ExpandableHashMap<KeyType, ValueType>::associate(const KeyType& key, const ValueType& value) {
	...
	m_table[bucketN].push_back(Node(key, value));
	m_nElements++;
	
	// checking whether a rehashing is required
	if (double(m_nElements) / m_table.size() > m_maxLoadFactor)
		rehashMap();
}
...
const ValueType* ExpandableHashMap<KeyType, ValueType>::find(const KeyType& key) const {
	...
	for (auto itr = m_table[bucketN].cbegin(); itr != m_table[bucketN].cend(); itr++)
		// key found
		if (itr->m_key == key)
			return &itr->m_value;
...
unsigned int ExpandableHashMap<KeyType, ValueType>::getBucketNumber(const KeyType& key, int tableSize) const {
	...
	unsigned int hasher(const KeyType& k);  // prototype of hash function
	unsigned int h = hasher(key); // hashing key
...
void ExpandableHashMap<KeyType, ValueType>::rehashMap() {
	...
	for (int i = 0; i != m_table.size(); i++)
		...
		for (auto itr = m_table[i].cbegin(); itr != m_table[i].cend();) {
			
			// storing next iterator
			auto nextItr = itr;
			nextItr++;

			// finding bucket number of new association
			unsigned int bucketN = getBucketNumber(itr->m_key, newSize);
			// splicing current node object into new list
			newTable[bucketN].splice(newTable[bucketN].cbegin(), m_table[i], itr);
				// current iterator is invalidated

			itr = nextItr;	// updating itr for next iteration
		}

	// swapping tables so the old one is destroyed as the function ends scope
