...
void Socrates::doSomething() {
	...
	int ch;		// character to hold user key press

	if (getWorld()->getKey(ch)) { // user hit a key during this tick!

		// updating the game according to the keypress
		switch (ch) {
			...
			case KEY_PRESS_RIGHT: {
				...
				int newPosAngle = getPositionalAngle();
				// Socrates moves five degrees counterclockwise
				if (ch == KEY_PRESS_LEFT)
					newPosAngle += 5;
				// Socrates moves five degrees clockwise
				else if (ch == KEY_PRESS_RIGHT)
					newPosAngle -= 5;

				// moving socrates and updating direction
				double newX = VIEW_RADIUS * (1 + cos(newPosAngle * 1.0 / 360 * 2 * PI));
				double newY = VIEW_RADIUS * (1 + sin(newPosAngle * 1.0 / 360 * 2 * PI));
					// (newX, newY)  = (R + Rcos(theta), R + Rsin(theta))
				moveTo(newX, newY);
				setDirection(newPosAngle + 180);
				break;
			}
