...
int StudentWorld::init() {
    ...
    for (set<DirtPile*>::iterator it = dirtpiles.begin(); it != dirtpiles.end(); it++)
        m_actors.insert(*it);
...
bool StudentWorld::findAndEatFood(double x, double y) {
    for (set<Actor*>::iterator it = m_actors.begin(); it != m_actors.end(); it++) {
...
Direction StudentWorld::getDirToSocrates(double x, double y) const{
    return atan2(getSocratesY() - y, getSocratesX() - x) * 360 / (2 * PI);
}

// ################################################################################################################
// RETRIEVING INFORMATION ABOUT WORLD AND WORLD OBJECTS
// ################################################################################################################
...
bool StudentWorld::findClosestFoodDir(double x, double y, Direction& dir) const {
    ...
    bool foodFound = false;                        // to track whether a food object has been found within VIEW_RADIUS pixels of (x, y)
    ...
    for (set<Actor*>::iterator it = m_actors.begin(); it != m_actors.end(); it++) {
        ...
        if ((*it)->actorEatable()) {
            ...
            if (foodDist <= closestDist) {
                // update food found
                foodFound = true;
                // update closest distance and closest direction
...
void StudentWorld::findValidCoords(int &x, int &y) {
...
string StudentWorld::getStatText() {
