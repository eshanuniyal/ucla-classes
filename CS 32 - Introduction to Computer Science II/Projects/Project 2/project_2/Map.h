...
class Map
{
  ...
  private:
    ...
    struct Pair
    {
        KeyType   m_key;
        ValueType m_value;
        Pair* m_next; // pointer to next pair
        Pair* m_prev; // pointer to previous pair
    };

    int  m_size;    // number of pairs in the map
    Pair* m_head;   // head of doubly-linked list of Pair objects
    Pair* m_tail;   // tial of doubly-linked list of Pair objects   
